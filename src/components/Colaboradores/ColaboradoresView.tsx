...
export const ColaboradoresView = () => {
  ...
  const {
    ...
    setSearchTerm,
    filteredItems
  } = useSearch(colaboradores, ['nome', 'email', 'nivel_acesso']);
  const carregarColaboradores = async () => {
    try {
      const {
        data,
        error
      } = await supabase.from('colaboradores').select('*').order('created_at', {
        ascending: false
      });
      if (error) {
        throw error;
  ...
  const confirmarDelecao = async () => {
    ...
    try {
      const {
        error
      } = await supabase.from('colaboradores').delete().eq('id', colaboradorToDelete.id);
      if (error) {
        throw error;
      }
      toast({
  ...
  return <div className="space-y-6 sm:space-y-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        {canCreateContent && <Button variant="hero" size="lg" onClick={() => setModalOpen(true)} className="w-full sm:w-auto">
      ...
      <div className="flex flex-col sm:flex-row gap-4">
        <div className="relative flex-1">
          ...
          <Input placeholder="Buscar membros da equipe..." className="pl-10 bg-background border-border" value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        </div>
      </div>
      ...
      <Card className="bg-card border border-border shadow-card">
        ...
        <div className="p-6">
          {loading ? <div className="flex justify-center items-center py-8">
              ...
              {canCreateContent && <Button onClick={() => setModalOpen(true)} className="mt-4">
            ...
            return <div key={colaborador.id} className={`p-6 rounded-xl border hover:shadow-lg transition-all duration-300 ${isAniversarioProximo ? 'bg-gradient-to-r from-yellow-50 to-orange-50 border-yellow-200' : 'bg-card border-border'}`}>
                      ...
                      <div className="flex items-center space-x-4 min-w-0 flex-1">
                        ...
                        <div className="min-w-0 flex-1">
                          ...
                          <p className="text-sm font-medium text-primary mb-3">
                            {colaborador.nivel_acesso === 'admin' ? 'Administrador' : colaborador.nivel_acesso === 'gestor_trafego' ? 'Gestor de Tráfego' : 'Customer Success'}
                          </p>
                          ...
                          <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-3">
                            ...
                            <div className={`flex items-center space-x-2 rounded-lg p-2 ${isAniversarioProximo ? 'bg-yellow-100 border border-yellow-200' : 'bg-muted/30'}`}>
                              <Cake className={`h-4 w-4 flex-shrink-0 ${isAniversarioProximo ? 'text-yellow-600' : 'text-pink-500'}`} />
                              <div>
                                <p className="text-xs font-medium text-muted-foreground uppercase tracking-wider">Aniversário</p>
                                <p className={`text-sm font-medium ${isAniversarioProximo ? 'text-yellow-700' : 'text-foreground'}`}>
                      ...
                      <div className="flex items-center justify-end lg:justify-center">
                        {isAdmin && <DropdownMenu>
                            ...
                            <DropdownMenuContent align="end" className="w-48">
                              ...
                              <DropdownMenuItem onClick={() => handleDeletarColaborador(colaborador)} className="text-destructive focus:text-destructive">
      ...
      <NovoColaboradorModal open={modalOpen} onOpenChange={setModalOpen} onSuccess={() => {
      carregarColaboradores();
    }} />

      {/* Modal de Editar Colaborador */}
      <EditarColaboradorModal open={editModalOpen} onOpenChange={setEditModalOpen} colaborador={selectedColaborador} onSuccess={() => {
      carregarColaboradores();
    }} />

      {/* Modal de Alterar Senha */}
      <AlterarSenhaModal open={senhaModalOpen} onOpenChange={setSenhaModalOpen} colaborador={selectedColaborador} onSuccess={() => {
      carregarColaboradores();
    }} />
      ...
      <AlertDialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
        <AlertDialogContent>
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction onClick={confirmarDelecao} className="bg-destructive text-destructive-foreground hover:bg-destructive/90">
              Excluir
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
