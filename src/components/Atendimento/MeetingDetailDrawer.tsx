...
import { NativeMeetingEditor } from "./NativeMeetingEditor";
                ...
                <div className="flex flex-col gap-5 px-6 py-5 flex-1">
                    ...
                    <div>
                        <div className="flex items-center gap-2 mb-2">
                            ...
                            <span className="text-sm font-semibold">Pauta da Reunião</span>

                            <Button
                                variant="ghost"
                                size="sm"
                                className="h-6 text-xs ml-auto gap-1 text-muted-foreground"
                                onClick={() => {
                                    saveDetails.mutate({
                                        titulo: event.summary ?? undefined,
                                        dataEvento: startRaw ?? undefined,
                                    });
                                    const url = `${window.location.origin}/pauta/${event.id}?n=${encodeURIComponent(event.summary || "Reunião")}`;
                                    navigator.clipboard.writeText(url);
                                    toast({ title: "Link copiado", description: "O link público da pauta está na área de transferência." });
                                }}
                        ...
                        <NativeMeetingEditor
                            ...
                            initialContent={stored?.pauta_html || ""}
                            isSaving={saveDetails.isPending}
                            onContentChange={(html) => {
                                if (savePautaTimeoutRef.current) clearTimeout(savePautaTimeoutRef.current);
                                savePautaTimeoutRef.current = setTimeout(() => {
                                    saveDetails.mutate({
                                        pauta_html: html,
                                        titulo: event.summary ?? undefined,
                                        dataEvento: startRaw ?? undefined,
