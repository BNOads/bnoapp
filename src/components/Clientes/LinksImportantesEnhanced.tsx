...
export const LinksImportantesEnhanced = ({ clienteId, isPublicView = false }: LinksImportantesProps) => {
  ...
  const { searchTerm, setSearchTerm, filteredItems: filteredLinks } = useSearch(
    links,
    ['titulo', 'url']
  );
          ...
          <div className="flex items-center justify-between mb-3">
            ...
            {isAuthenticated && (
              <Button 
                size="sm" 
                variant="ghost" 
                onClick={() => buscarDashboardsAutomaticos()}
          ...
          {filteredLinks.length === 0 ? (
            <div className="text-center py-8 text-muted-foreground">
              {searchTerm ? (
                ...
                <p>Nenhum link manual adicionado</p>
              ) : null}
            </div>
          ) : (
            <>
        ...
        <DialogContent>
          ...
          {editingLink && (
            <div className="space-y-4">
              ...
              />
              <Select
                value={editingLink.tipo}
                onValueChange={(value) => setEditingLink({ ...editingLink, tipo: value })}
              >
                <SelectTrigger>
                  <SelectValue placeholder="Tipo do link" />
                </SelectTrigger>
