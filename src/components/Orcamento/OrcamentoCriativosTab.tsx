...
import { VincularOrcamentoCriativosModal } from "./VincularOrcamentoCriativosModal";

interface OrcamentoCriativosTabProps {
    orcamentoId: string;
    clienteId: string;
}
...
export function OrcamentoCriativosTab({ orcamentoId, clienteId }: OrcamentoCriativosTabProps) {
    ...
    const loadLinkedCreatives = async () => {
        try {
            setLoading(true);
            // 1. Get linked folder names from the new table
    ...
    return (
        <div className="space-y-4">
            ...
            <VincularOrcamentoCriativosModal
                open={showManageModal}
                onOpenChange={setShowManageModal}
                orcamentoId={orcamentoId}
                clienteId={clienteId}
                onSuccess={loadLinkedCreatives}
            />
        </div>
    );
}
