...
export const LaunchFieldOptionsManager = ({ open, onOpenChange, onUpdated }: { open: boolean; onOpenChange: (o: boolean) => void; onUpdated?: () => void }) => {
  const [selectedField, setSelectedField] = useState<FieldKey>('tipo_lancamento');
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      const { error } = await supabase.from('launch_field_options').delete().eq('id', id);
      if (error) throw error;
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>Gerenciar Opções de Lançamentos</DialogTitle>
          <DialogDescription>Editar labels e cores de todas as opções de dropdown de lançamentos</DialogDescription>
        </DialogHeader>
...
export default LaunchFieldOptionsManager;
