        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center space-x-2 sm:space-x-4">
            ...
            <nav className="hidden lg:flex space-x-2">
              {tabs.map(tab => {
              const Icon = tab.icon;
              return <Button key={tab.id} variant={activeTab === tab.id ? "hero" : "ghost"} size="default" onClick={() => onTabChange(tab.id)} className="flex items-center space-x-2">
            ...
            <div className="lg:hidden">
              <DropdownMenu>
                ...
                <DropdownMenuContent align="end" className="w-48">
                  {tabs.map(tab => {
                  const Icon = tab.icon;
                  return <DropdownMenuItem key={tab.id} onClick={() => onTabChange(tab.id)} className={`flex items-center space-x-2 ${activeTab === tab.id ? "bg-accent" : ""}`}>
                        <Icon className="h-4 w-4" />
                        <span>{tab.label}</span>
                      </DropdownMenuItem>;
                })}
