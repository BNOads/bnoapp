                                                ...
                                                const pushRes = await fetch(
                                                    `${supabaseUrl}/functions/v1/send-push-notification`,
                                                    {
                                                        ...
                                                        body: JSON.stringify({
                                                            ...
                                                            titulo: `⚠️ Saldo baixo Meta Ads - ${clientName}`,
                                                            conteudo: `Conta ${accountName}: saldo ${formatCurrencyBRL(balance)} (limite: ${formatCurrencyBRL(LOW_BALANCE_THRESHOLD)})`,
                                                            destinatarios,
                                                        }),
                                                    }
                                                );
                                                const pushJson = await pushRes.json().catch(() => ({}));
                                                console.log(`Push sent for low-balance ${metaId}: sent=${pushJson.sent ?? '?'} failed=${pushJson.failed ?? '?'}`, pushJson.errors ?? '');
                                            } catch (pushErr) {
                                                console.error(`Failed to send push for low-balance ${metaId}:`, pushErr);
                                            }
                                        }
                                    }
                                }
