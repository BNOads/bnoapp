...
-- Atualizar uma gravação existente com transcrição de exemplo
...
-- Se não houver gravação da Paloma, criar uma
...
WHERE NOT EXISTS (
  SELECT 1 FROM public.gravacoes WHERE titulo ILIKE '%paloma%'
);

-- Inserir uma reunião de exemplo também
...
-- Atualizar índices de busca para os registros inseridos
